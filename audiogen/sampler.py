...
def sample_all(generators, *args, **kwargs):
    '''Convert list of audio waveform generators into list of
       packed sample generators.'''
...
class NonSeekableFileProxy(object):
    def __init__(self, file_instance):
        '''Proxy to protect seek and tell methods of
...
def write_wav(f,
    ...
    channel_count = 1 if hasattr(channels, '__next__') else len(channels)
    ...
    w.setparams((
        ...
        sample_width,
        FRAME_RATE,
        0,  # setting 0 frames, should update automatically as frames written
        COMPRESSION_TYPE,
        COMPRESSION_NAME
        ))
...
def play(channels, blocking=True, raw_samples=False):
    ...
    dependency, must be installed for this feature to work.
    '''
    if not pyaudio_loaded:
        raise Exception("Soundcard playback requires PyAudio. "
    ...
    logger.debug(f"format = {p.get_format_from_width(SAMPLE_WIDTH)}")
    logger.debug(f"channels = {channel_count}")
    logger.debug(f"rate = {FRAME_RATE}")
